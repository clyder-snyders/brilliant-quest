...
const COMMAND_LABELS: Record<string, string> = {
  ...
  ifElse: '❓ If/Else',
  setVariable: '📝 Set Var',
  changeVariable: '📝 Change Var',
  compareVariable: '📝 Compare Var',
  defineFunction: '📦 Define Func',
...
const COMMAND_CATEGORIES = [
  ...
  { name: 'Variables', commands: ['setVariable', 'changeVariable', 'compareVariable'], unlocksAt: 25 },
  { name: 'Functions', commands: ['defineFunction', 'callFunction'], unlocksAt: 35 },
  { name: 'Logic', commands: ['andOp', 'orOp', 'notOp'], unlocksAt: 39 },
];
...
export default function GameScreen() {
  ...
  const expandCommands = (cmds: string[]): string[] => {
    ...
    for (let i = 0; i < cmds.length; i++) {
      const cmd = cmds[i];
      if (cmd === 'repeat2') {
        // Repeat next command 2 times (or last movement if at end)
        ...
        // Add up to 30 moveForward1 as a safety limit
        for (let j = 0; j < 30; j++) expanded.push('moveForward1_untilGoal');
      } else {
        expanded.push(cmd);
      }
    }
  ...
  const runSequence = useCallback(async () => {
    ...
    for (let i = 0; i < expanded.length; i++) {
      ...
      if (cmd.startsWith('moveForward') || cmd === 'moveForward1_untilGoal') {
        ...
        for (let s = 0; s < steps; s++) {
          ...
          if (isWalkable(nx, ny)) {
            ...
            if (isGoal(nx, ny)) {
              ...
              const result = calculateScore(timer, sequence.length, level.parTime, level.parCommands, hintsUsed, hits, usedLoops, usedFuncs);
              dispatch({
                type: 'COMPLETE_LEVEL',
                levelId: level.id,
                progress: { completed: true, stars: result.stars, bestScore: result.score, bestTime: timer },
              });
              // Navigate to result
            ...
            setWallHits(h => h + 1);
            setWallHitAnim(true);
            setTimeout(() => setWallHitAnim(false), 300);
            await delay(400);
            break;
          }
        }
      ...
      if (cmd === 'moveForward1_untilGoal' && isGoal(pos.x, pos.y)) break;
    }
    ...
    // eslint-disable-next-line
  }, [sequence, level, timer, hintsUsed, dispatch]);
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'hsl(220, 27%, 98%)' }}>
      {/* Top Bar */}
      <div className="flex items-center h-14 px-4 border-b shrink-0" style={{ background: 'white', borderColor: 'hsl(214, 32%, 91%)' }}>
        <button className="btn-secondary py-1 px-3 text-xs" onClick={() => { runningRef.current = false; dispatch({ type: 'SET_SCREEN', screen: 'levelMap' }); }}>← Map</button>
        <div className="flex-1 text-center">
          <span className="font-bold" style={{ color: 'hsl(217, 33%, 17%)' }}>Level {level.id} — {level.name}</span>
          {state.practiceMode && <span className="ml-2 px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: 'hsl(25, 95%, 93%)', color: 'hsl(25, 95%, 53%)' }}>PRACTICE</span>}
        </div>
        <div className="flex items-center gap-4 text-sm">
          <span style={{ color: timer > level.parTime * 2 ? 'hsl(0, 84%, 60%)' : 'hsl(215, 16%, 47%)' }}>⏱ {formatTime(timer)}</span>
          <span style={{ color: 'hsl(215, 16%, 47%)' }}>Moves: {sequence.length}/{level.maxCommands}</span>
        </div>
      </div>
        ...
        <div className="w-[260px] shrink-0 border-l flex flex-col overflow-hidden" style={{ background: 'white', borderColor: 'hsl(214, 32%, 91%)' }}>
          ...
          <div className="h-[45%] flex flex-col p-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button className="btn-primary text-xs py-2" onClick={runSequence} disabled={isRunning || sequence.length === 0} style={{ opacity: isRunning || sequence.length === 0 ? 0.5 : 1 }}>
                ▶ Run
              </button>
              <button className="btn-secondary text-xs py-2" onClick={undoCommand} disabled={isRunning} style={{ opacity: isRunning ? 0.5 : 1 }}>
                ↩ Undo
              </button>
              <button className="btn-secondary text-xs py-2" onClick={clearSequence} disabled={isRunning} style={{ opacity: isRunning ? 0.5 : 1 }}>
                ⊘ Clear
              </button>
              <button className="btn-secondary text-xs py-2" onClick={handleReset} style={{}}>
                ↺ Reset
              </button>
            </div>
          </div>
